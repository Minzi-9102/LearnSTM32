...
{
	if(SysTick_Config(18000000/1000))					//配置系统滴答定时器0.25ms中断一次（计算公式：中断周期=1/72*（SysTick_Config函数参数））
	{
...
{
	s_iTimDelayCnt = nms*4;										//将延时计数器s_iTimDelayCnt的数值赋值为nms*4
																						//因为现在滴答计时器0.25ms中断一次，所以需要经过4x次中断才是延时x ms
