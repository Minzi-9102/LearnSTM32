...
static	u8	s_arrRecBuf[UART2_BUF_SIZE];						//接受串口循环队列的缓冲区
...
static	void	ConfigUART(u32 bound)
{
	...
	//配置NVIC
	NVIC_InitStructure.NVIC_IRQChannel = USART2_IRQn;						//中断信号通道
...
void USART2_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART2,USART_IT_RXNE)!= RESET)//接收缓冲区非空中断
	{
		...
		uData = USART_ReceiveData(USART2);//将接收到的数据保存的uData
	...
	{
		USART_ClearITPendingBit(USART2,USART_IT_TXE);//清空发送中断标志
		NVIC_ClearPendingIRQ(USART2_IRQn);//清除中断挂起
