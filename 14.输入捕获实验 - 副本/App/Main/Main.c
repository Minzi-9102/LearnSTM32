...
{   
	...
	static i16 count = 0;
	static double sumCaptureTime = 0.0;
	char buffer[50];
	i32 captureVal;
	float captureTime;
...
//			ScanKeyOne(KEY_NAME_KEY3,ProcKeyUpKey3,ProcKeyDownKey3);
			...
			sumCaptureTime += captureTime;
			//printf("counted-%d,time-%0.2fms,sum-%0.2fms\r\n",count,captureTime,sumCaptureTime);
			if(count == 10)
			{	
				sprintf(buffer, "%0.2f  ", sumCaptureTime/10);  // 保留2位小数
				OLEDShowString(32,32,buffer);
				printf("avg-%0.2fms\r\n",sumCaptureTime/10);
				sumCaptureTime = 0.0;
				count = 0;
			}
...
{ 
  ...
  printf("Init System has been finished.\r\n" );  //打印系统状态
	SetPWM(300);
	OLEDShowString(8,0,"PWM avg high level period");
  while(1)
  {
    Proc2msTask();  //2ms处理任务
    Proc1SecTask(); //1s处理任务   
