...
void TIM5_IRQHandler(void)
{
	if((s_iCaptureSts & 0x80) == 0)//最高位为0，表示捕获还未完成
	{
		...
		{
			if(s_iCaptureSts & 0x40)//bit6为1，即上次捕获到上升沿，则这次捕获到下降沿
			{
				...
				//CC1P = 1，设置为上升沿捕获，为下次捕获做准备
				TIM_OC1PolarityConfig(TIM5,TIM_ICPolarity_Rising);//20250801
			}
			else//bit6为0，表示未捕获到上升沿，这是第一次捕获到上升沿
			{
				...
				TIM_OC1PolarityConfig(TIM5,TIM_ICPolarity_Falling);//CC1P = 0，设置为下降沿捕获//20250801
			}
		}
	}
