...
*********************************************************************************************************/
 ...
 //表示整个OLED屏幕的所有像素，128表示横向列，共128列，8表示纵向页，共8页，
 //每一页都由一个u8类型数据组成
 //即每一列有8页，每一页有8行（8个像素点）
 //选定x，y坐标，写入一个u8数据，就确定了从(x,y)到(x,y+7)的像素点是否点亮
/*********************************************************************************************************
*                                              内部函数声明
*********************************************************************************************************/
...
static void OLEDDrawPoint(u8 x,u8 y,u8 t);//向OLED屏指定位置画点
...
* 函数名称：OLEDDrawPoint
* 函数功能：
* 输入参数：u8 x，横坐标,u8 y，纵坐标,u8 t，点亮/熄灭控制位
* 输出参数：
* 返 回 值：
* 创建日期：2018年01月01日
* 注    意：
...
static void OLEDDrawPoint(u8 x,u8 y,u8 t)
{
	...
	page = 7-y/8;//求指定位置所在页数
	bx = y%8;//求指定位置在上面求出页数中的行号
	temp = 1<<(7-bx);//（7-bx）求出相应SSD1306的行号，并将字节中相应的位置为1
	
	//因为 SSD1306 的 Page 内 8 行 在字节里的排列是 bit7 → bit0，
	//而 bx = y % 8 得到的是 “从页顶开始数的行号”（0~7）。
	//所以 7-bx 就把“行号”映射成 “字节里的 bit 位置”（仍是 7~0）。
	
	if(t)//判断填充标志为1还是0
	{
		s_arrOLEDGRAM[x][page]|=temp;//如果填充标志为1，指定点填充
	}
	else{
		s_arrOLEDGRAM[x][page] &= ~temp;//如果填充标志为0，指定点清空
	}
}
...
void OLEDRefreshGRAM(void)
{
	...
	for(i = 0;i<8;i++)//遍历每一页
	{
		...
		OLEDWriteByte(0x10,OLED_CMD);//设置显示位置一列高地址0x10中的1表示为高地址
		//例如写0x0f,0x19,则写完后列地址标记为0x9f
		for(n = 0;n<128;n++)//遍历每一列
		{//通过循环将stm32的GRAM写入SSD1306的GRAM
			//每次写入当前页（i）中第n列的全部8行数据
			//即写入选中x和y坐标后的（x，y）到（x，y+7）个像素
...
void OLEDShowNum(u8 x,u8 y,u32 num,u8 len,u8 size)
{
	...
	u8 enshow = 0;//区分0是否为高位0（有效数字前的0）标志
	
	for(t = 0;t<len;t++)
	{
		temp = (num/CalcPow(10,len-t-1))%10;//根据显示位数（宽度）len，从高到低取出要显示数字的各个位，存到temp中
		if(enshow == 0 && t<(len - 1))//从最高位开始显示，如果标记enshow为0并且还未取到最后一位
		{
			if(temp == 0)//如果temp等于0
			{
