...
static void ConfigRCC(void)
{
	ErrorStatus HSEStartUpStatus;//定义枚举变量HSEStartUpStatus，用来标志外部高速晶振状态
	...
	if(HSEStartUpStatus == SUCCESS)//外部高速晶振成功稳定
	{
		...
		FLASH_PrefetchBufferCmd(FLASH_PrefetchBuffer_Enable);//使能FLASH预读取缓冲区，加速内部FLASH读取
		FLASH_SetLatency(FLASH_Latency_2);//设置代码延时值，FLASH_Latency_2，2延时周期
		RCC_HCLKConfig(RCC_SYSCLK_Div1);
		//设置高速AHB时钟（HCLK），RCC_SYSCLK_Div1，HCLK = SYSCLK = 72MHz
//		RCC_PCLK2Config(RCC_HCLK_Div1);
		RCC_PCLK2Config(RCC_HCLK_Div2);
		//设置高速APB2时钟（PCLK2），RCC_HCLK_Div2，PCLK2 = HCLK/2 = 36MHz
		//这时降低了PCLK2时钟频率，会导致串口波特率改变，因为串口波特率计算公式为fPCLK2 /（16*USARTDIV）
//		RCC_PCLK1Config(RCC_HCLK_Div2);
		RCC_PCLK1Config(RCC_HCLK_Div4);
		//设置低速APB1时钟（PCLK1），RCC_HCLK_Div2，PCLK1 = HCLK/4 = 18MHz
		//RCC_ADCCLKConfig(RCC_PCLK2_Div4);
		//设置ADC时钟（ADCCLK），RCC_PCLK2_Div4，ADCCLK = PCLK2/4
